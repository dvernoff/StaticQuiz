		...
		#pragma semicolon 1				// Исключительно
		#pragma newdecls required		// новый синтаксис.
...
public Action CommandSay(int i, any args)
{
    if(game == true)
    {
        ...
        if(StrEqual(Arg, Otvet))
        {
			...
			{
				CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_WRONG");
				return;
			}
			...
			if(StrEqual(Type, "shop", false))
			{
				...
				CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_PLAYER_WIN", i, iNagrada, TypeText, Otvet);
				StopGame();
			}
			...
			else if(StrEqual(Type, "vip", false))
			{
				...
				{
					CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_VIP_YES");
				}
				else
				{
					VIP_GiveClientVIP(1, i, iNagrada, TypeText, true);
					CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_PLAYER_VIPWIN", i, Type, TypeText, iNagrada, Otvet);
					StopGame();
				}
			}
			else
			{
				CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_ERRORFATAL");
				//FATAL ERROR
			}
        }
    }
...
public Action r_Cmd(int i, any args)
{
	if(!CheckFlag(i, admin_flag))
	{
		...
		if(StrEqual(Arg, "stop"))
		{
			if(game == true)
			{
				CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_ADMIN_CANCEL", i);
				StopGame();
			}
			else
			{
				CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_INGAME");
			}
			return Plugin_Handled;
		}
		if(game == true)
		{
			CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_ADMIN_RETRY", i, Vopros, iNagrada, TypeText);
			return Plugin_Handled;
		}
		if(args > 4 || args < 4)
		{
			CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_COMMAND_ERROR");
			return Plugin_Handled;
		}
		...
		if(strl > 256)
		{
			CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_MAXARGS");
			return Plugin_Handled;
		}
		...
		{
			CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_COMMAND_2ERROR", maxaward);
			return Plugin_Handled;
		}
		...
		if(StrEqual(Type, "lvl"))
		{
		   ...
		   {
			   CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_TYPE_LVL_ERROR");
			   return Plugin_Handled;
		   }
		}
		else if(StrEqual(Type, "shop"))
		{
		   ...
		   {
			   CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_TYPE_SHOP_ERROR");
			   return Plugin_Handled;
		   }
		}
		else
		{
			...
			if(uti_puti != -1)
			{
				if(vip == true)
				{
					...
					{
						CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_ERRVIPGR", TypeText);
						return Plugin_Handled;
					}
				}
				else
				{
					CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_TYPE_VIP_ERROR");
					return Plugin_Handled;
				}
			}
			else
			{
				CGOPrintToChat(i, "%t %t", "CHAT_PREFIX", "CHAT_TYPE_ARG_ERROR");
				return Plugin_Handled;
			}
		}
		...
		{
			CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_START_VIPGAME", i, Vopros, Type, TypeText, iNagrada);
		}
		else
		{
		 	CGOPrintToChatAll("%t %t", "CHAT_PREFIX", "CHAT_START_GAME", i, Vopros, iNagrada, TypeText);
		}
		game = true;
	}
	else
